...
class FillKirovogradTable extends Command
{
    protected $signature = 'kirovograd:fill {street?}';
    protected $description = 'Fill kirovograd table with data from residence_data and personal_data';
    ...
    public function handle()
    {
        ...
        $this->info('Starting to fill kirovograd table for street: ' . ($street === '%' ? 'all streets' : $street));

        if ($street === '%') {
            DB::table('kirovograd')->truncate();
            $this->info('Table kirovograd truncated.');
        }
        ...
        $query = DB::table('residence_data')
            ...
            ->where('residence_data.region_id', 25)
            ->whereRaw('LOWER(TRIM(residence_data.residence_locality)) = ?', [mb_strtolower('КІРОВОГРАД', 'UTF-8')])
            ->orderBy('residence_data.drfo_new'); // Добавляем сортировку
        ...
        $query->chunk($chunkSize, function ($rows) use (&$totalRows, &$errorRows) {
            ...
            if (!empty($insertData)) {
                try {
                    DB::table('kirovograd')->insert($insertData);
        ...
        if ($totalRows === 0) {
            $this->error('No valid data found for region_id=25 and residence_locality=КІРОВОГРАД' . ($street !== '%' ? " and street=$street" : ''));
            return;
        }

        $this->info('Kirovograd table filled successfully with ' . $totalRows . ' rows.');
        if ($errorRows > 0) {
